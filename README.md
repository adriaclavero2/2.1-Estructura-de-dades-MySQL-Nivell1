...
The goal is to practice conceptual modeling, logical modeling, and physical implementation using SQL, ensuring data integrity, normalization, and correct relationship handling.
...
Business rule:  
A brand of glasses is supplied by **one single supplier**, but a supplier may provide **multiple brands**.
...
Locality and province are stored in **separate tables**:  
...
- Total price  
- One client can make **many orders**, but each order belongs to **one client**
...
- Unique identifier  
- Name  
- A pizza belongs to **one category**, but a category may contain **many pizzas**
...
- Use of SQL `ENUM` for controlled values (e.g., delivery type, employee role)  
...
2. Execute the table‑creation scripts in MySQL Workbench.  
3. Insert sample data if needed.  
4. Run validation queries to ensure the model behaves correctly.  
5. Use ERD diagrams to visualize and justify design decisions.
